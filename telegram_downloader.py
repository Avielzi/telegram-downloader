"""
Telegram Desktop Downloader v2.3.0 ULTIMATE PRO
...
from PyQt6.QtWidgets import (
    ...
    QLabel, QLineEdit, QSpinBox, QComboBox, QGroupBox, QProgressBar, 
    QScrollArea, QFrame, QStackedWidget, QFormLayout, QInputDialog, 
    QMessageBox, QMenu, QFileDialog
)
from PyQt6.QtCore import (
    QThread, pyqtSignal, Qt, QSettings, QSize, QTimer, 
...
QLineEdit, QSpinBox, QComboBox { border-radius: 6px; padding: 8px; }
QGroupBox { font-weight: bold; border-radius: 8px; margin-top: 15px; padding-top: 15px; }
QProgressBar { border-radius: 10px; text-align: center; height: 20px; }
...
QLineEdit, QSpinBox, QComboBox { border: 2px solid #dee2e6; background-color: white; color: #333; }
QGroupBox { border: 1px solid #dee2e6; color: #333; }
QProgressBar { border: 1px solid #dee2e6; background-color: #e9ecef; }
QFrame#mediaItem { background-color: white; border: 1px solid #e9ecef; }
QLabel { color: #333; }
"""

DARK_STYLE = COMMON_STYLE + """
QMainWindow { background-color: #121212; color: #e0e0e0; }
QPushButton#successBtn { background-color: #2e7d32; }
QPushButton#dangerBtn { background-color: #c62828; }
QLineEdit, QSpinBox, QComboBox { border: 2px solid #333; background-color: #1e1e1e; color: white; }
QGroupBox { border: 1px solid #333; color: #0088cc; }
QProgressBar { border: 1px solid #333; background-color: #1e1e1e; }
...
class ParallelDownloadThread(QThread):
    """ULTIMATE Parallel Downloader with Optimized Speed and ETA"""
...
class MediaItemWidget(QFrame):
    ...
    def init_ui(self):
        ...
        hover = "#252525" if self.is_dark else "#f1f3f5"
        ...
        self.checkbox.stateChanged.connect(lambda: self.changed.emit())
        layout.addWidget(self.checkbox)
        
        icon_map = {'photo': '📷', 'image': '🖼️', 'video': '🎬', 'document': '📄', 'archive': '📦', 'file': '📎'}
        self.icon_lab = QLabel(icon_map.get(self.item['type'], '📎'))
        self.icon_lab.setFont(QFont("Arial", 16))
        layout.addWidget(self.icon_lab)
        
        info_l = QVBoxLayout()
        name_lab = QLabel(self.item['name'])
        name_lab.setStyleSheet(f"font-weight: bold; color: {'#fff' if self.is_dark else '#343a40'};")
        info_l.addWidget(name_lab)
        
        meta_l = QHBoxLayout()
        size_lab = QLabel(humanize.naturalsize(self.item['size']))
        size_lab.setStyleSheet("color: #6c757d; font-size: 11px;")
        meta_l.addWidget(size_lab)
        
        date_lab = QLabel(self.item['date'].strftime("%d/%m/%Y"))
        date_lab.setStyleSheet("color: #888; font-size: 11px;")
        meta_l.addWidget(date_lab)
        meta_l.addStretch()
...
class ModernWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.main_l = QVBoxLayout(central)
        self.main_l.setContentsMargins(0, 0, 0, 0)
        self.main_l.setSpacing(0)
        
        header = QFrame()
        header.setFixedHeight(70)
        header.setStyleSheet(f"background-color: {'#1e1e1e' if self.is_dark else 'white'}; border-bottom: 1px solid {'#333' if self.is_dark else '#dee2e6'};")
        h_l = QHBoxLayout(header)
        h_l.addWidget(QLabel(f"⚡ {tr('app_name')} <span style='color: #0088cc;'>ULTIMATE</span>", font=QFont("Arial", 18, QFont.Weight.Bold)))
        h_l.addStretch()
        
        self.theme_btn = QPushButton("🌙" if not self.is_dark else "☀️")
        self.theme_btn.setFixedSize(40, 40)
        self.theme_btn.clicked.connect(self.toggle_theme)
        h_l.addWidget(self.theme_btn)
        
        self.menu_btn = QPushButton("⚙️")
        self.menu_btn.setFixedSize(40, 40)
        self.menu_btn.clicked.connect(self.show_settings_menu)
        h_l.addWidget(self.menu_btn)
        self.main_l.addWidget(header)
        
        self.steps_w = self.create_steps_indicator()
        self.main_l.addWidget(self.steps_w)
        self.stack = QStackedWidget()
        self.main_l.addWidget(self.stack)
        ...
        self.aid = self.settings.value('api_id')
        self.ah = self.settings.value('api_hash')
        self.ph = self.settings.value('phone')
    ...
    def show_settings_menu(self):
        menu = QMenu(self)
        lang_m = menu.addMenu(tr("menu_language"))
        for c, n in self.translator.get_available_languages().items():
            a = lang_m.addAction(n)
            a.triggered.connect(lambda checked, code=c: self.change_lang(code))
        menu.addSeparator()
        menu.addAction("Logout", self.logout)
        menu.addAction(tr("menu_about"), self.show_about)
        menu.exec(self.menu_btn.mapToGlobal(self.menu_btn.rect().bottomLeft()))

    def change_lang(self, c):
        self.settings.setValue('language', c)
        self.translator.set_language(c)
        self.init_ui()

    def create_steps_indicator(self):
        w = QFrame()
        w.setFixedHeight(80)
        w.setStyleSheet(f"background-color: {'#121212' if self.is_dark else '#f8f9fa'}; border-bottom: 1px solid {'#333' if self.is_dark else '#dee2e6'};")
        l = QHBoxLayout(w)
        ...
        for i, name in enumerate([tr("step_setup"), tr("step_scan"), tr("step_select"), tr("step_download")]):
            ...
            c.setStyleSheet("background: #dee2e6; border-radius: 15px; color: white; font-weight: bold;")
            sl.addWidget(c, 0, Qt.AlignmentFlag.AlignCenter)
            lab = QLabel(name)
            lab.setFont(QFont("Arial", 9))
            lab.setStyleSheet("color: #6c757d;")
            sl.addWidget(lab, 0, Qt.AlignmentFlag.AlignCenter)
            self.step_widgets.append((c, lab))
            l.addWidget(sw)
            if i < 3:
                line = QFrame()
    ...
    def create_setup_page(self):
        ...
        card.setStyleSheet(f"background: {'#1e1e1e' if self.is_dark else 'white'}; border-radius: 15px; border: 1px solid {'#333' if self.is_dark else '#dee2e6'};")
        cl = QVBoxLayout(card)
        ...
        ob = QPushButton(tr("btn_open_telegram"))
        ob.setObjectName("primaryBtn")
        ob.clicked.connect(lambda: webbrowser.open('https://my.telegram.org/apps'))
        cl.addWidget(ob)
        form = QFormLayout()
        self.aid_in = QLineEdit(self.settings.value('api_id', ''))
        self.ah_in = QLineEdit(self.settings.value('api_hash', ''))
        self.ph_in = QLineEdit(self.settings.value('phone', ''))
        form.addRow(tr("label_api_id"), self.aid_in)
        form.addRow(tr("label_api_hash"), self.ah_in)
        form.addRow(tr("label_phone"), self.ph_in)
        cl.addLayout(form)
        sb = QPushButton(tr("btn_save_continue"))
        sb.setObjectName("successBtn")
        sb.setFixedHeight(50)
        sb.clicked.connect(self.save_and_continue)
        cl.addWidget(sb)
        l.addWidget(card)
        return p
    ...
    def create_scan_page(self):
        ...
        gb = QGroupBox(tr("scan_group_label"))
        gl = QVBoxLayout(gb)
        self.group_in = QLineEdit()
        self.group_in.setPlaceholderText(tr("scan_group_placeholder"))
        self.group_in.setFixedHeight(50)
        gl.addWidget(self.group_in)
        l.addWidget(gb)
        ob = QGroupBox(tr("scan_options"))
        ol = QHBoxLayout(ob)
        ol.addWidget(QLabel(tr("scan_max_messages")))
        self.max_spin = QSpinBox()
        self.max_spin.setRange(10, 200000)
        self.max_spin.setValue(1000)
        ol.addWidget(self.max_spin)
        l.addWidget(ob)
        self.scan_st = QLabel("")
        self.scan_st.setAlignment(Qt.AlignmentFlag.AlignCenter)
        l.addWidget(self.scan_st)
        self.scan_pr = QProgressBar()
        self.scan_pr.setVisible(False)
        l.addWidget(self.scan_pr)
        self.scan_btn = QPushButton(tr("btn_start_scan"))
        self.scan_btn.setObjectName("primaryBtn")
        self.scan_btn.setFixedHeight(60)
        self.scan_btn.clicked.connect(self.start_scan)
        l.addWidget(self.scan_btn)
        self.stop_scan_btn = QPushButton(tr("btn_stop_scan"))
        self.stop_scan_btn.setObjectName("dangerBtn")
        self.stop_scan_btn.setVisible(False)
        self.stop_scan_btn.clicked.connect(self.stop_scan)
        l.addWidget(self.stop_scan_btn)
        l.addStretch()
        return p
    ...
    def create_select_page(self):
        ...
        h = QHBoxLayout()
        h.addWidget(QLabel(tr("select_title"), font=QFont("Arial", 18, QFont.Weight.Bold)))
        h.addStretch()
        sa = QPushButton(tr("btn_select_all"))
        sa.clicked.connect(self.select_all)
        h.addWidget(sa)
        sn = QPushButton(tr("btn_select_none"))
        sn.clicked.connect(self.select_none)
        h.addWidget(sn)
        l.addLayout(h)
        
        fl = QHBoxLayout()
        fl.addWidget(QLabel(tr("filter_label")))
        self.filter_cb = QComboBox()
        self.filter_cb.addItems([tr("filter_all"), tr("filter_photos"), tr("filter_videos"), tr("filter_documents"), tr("filter_archives")])
        self.filter_cb.currentTextChanged.connect(self.apply_filter)
        fl.addWidget(self.filter_cb)
        self.search_in = QLineEdit()
        self.search_in.setPlaceholderText(tr("search_placeholder"))
        self.search_in.textChanged.connect(self.apply_filter)
        fl.addWidget(self.search_in)
        self.sel_lab = QLabel(tr("selected_count", count=0))
        self.sel_lab.setStyleSheet("font-weight: bold; color: #0088cc;")
        fl.addWidget(self.sel_lab)
        l.addLayout(fl)
        
        self.scroll = QScrollArea()
        self.scroll.setWidgetResizable(True)
        self.cont = QWidget()
        self.files_l = QVBoxLayout(self.cont)
        self.files_l.setSpacing(8)
        self.scroll.setWidget(self.cont)
        l.addWidget(self.scroll)
        
        path_l = QHBoxLayout()
        path_l.addWidget(QLabel(tr("download_path_label")))
        self.path_in = QLineEdit(str(Path.home() / "Downloads" / "Telegram"))
        path_l.addWidget(self.path_in)
        bb = QPushButton(tr("btn_browse"))
        bb.clicked.connect(self.browse_path)
        path_l.addWidget(bb)
        l.addWidget(QLabel(f"{tr('menu_settings')}: Parallel Downloads (3)"))
        l.addLayout(path_l)
        
        bl = QHBoxLayout()
        back = QPushButton(tr("btn_back"))
        back.setFixedWidth(120)
        back.clicked.connect(self.show_scan_page)
        bl.addWidget(back)
        self.dl_btn = QPushButton(tr("btn_download_selected"))
        self.dl_btn.setObjectName("successBtn")
        self.dl_btn.setFixedHeight(55)
        self.dl_btn.clicked.connect(self.start_download)
        bl.addWidget(self.dl_btn, 1)
        l.addLayout(bl)
        return p
    ...
    def create_download_page(self):
        ...
        l.setAlignment(Qt.AlignmentFlag.AlignCenter)
        l.setSpacing(20)
        self.dl_st = QLabel(tr("download_starting"), font=QFont("Arial", 18, QFont.Weight.Bold))
        l.addWidget(self.dl_st, 0, Qt.AlignmentFlag.AlignCenter)
        self.cur_f_lab = QLabel("")
        l.addWidget(self.cur_f_lab, 0, Qt.AlignmentFlag.AlignCenter)
        self.dl_pr = QProgressBar()
        self.dl_pr.setFixedWidth(600)
        l.addWidget(self.dl_pr, 0, Qt.AlignmentFlag.AlignCenter)
        self.stats_lab = QLabel("")
        l.addWidget(self.stats_lab, 0, Qt.AlignmentFlag.AlignCenter)
        self.speed_lab = QLabel("")
        self.speed_lab.setStyleSheet("color: #0088cc; font-weight: bold;")
        l.addWidget(self.speed_lab, 0, Qt.AlignmentFlag.AlignCenter)
        self.stop_dl_btn = QPushButton(tr("btn_stop_download"))
        self.stop_dl_btn.setObjectName("dangerBtn")
        self.stop_dl_btn.clicked.connect(self.stop_download)
        l.addWidget(self.stop_dl_btn, 0, Qt.AlignmentFlag.AlignCenter)
        
        done_l = QHBoxLayout()
        self.done_b = QPushButton(tr("btn_done"))
        self.done_b.setObjectName("primaryBtn")
        self.done_b.setFixedSize(180, 50)
        self.done_b.setVisible(False)
        self.done_b.clicked.connect(self.reset_to_scan)
        done_l.addWidget(self.done_b)
        self.open_f_b = QPushButton(tr("btn_open_folder"))
        self.open_f_b.setObjectName("successBtn")
        self.open_f_b.setFixedSize(180, 50)
        self.open_f_b.setVisible(False)
        self.open_f_b.clicked.connect(self.open_folder)
        done_l.addWidget(self.open_f_b)
        l.addLayout(done_l)
        return p
    ...
    def save_and_continue(self):
        ...
        if not all([aid, ah, ph]):
            return QMessageBox.warning(self, tr("error"), tr("error_empty_fields"))
        self.settings.setValue('api_id', aid)
        self.settings.setValue('api_hash', ah)
        self.settings.setValue('phone', ph)
        self.aid, self.ah, self.ph = aid, ah, ph
        self.init_telegram_client()
        self.show_scan_page()

    def init_telegram_client(self):
        self.sp = Path.home() / '.telegram_downloader' / 'session'
        self.sp.parent.mkdir(exist_ok=True)
        self.at = AuthThread(self.aid, self.ah, self.sp, self.ph)
        self.at.code_needed.connect(self.handle_code_request)
        self.at.password_needed.connect(self.handle_password_request)
        self.at.auth_success.connect(self.handle_auth_success)
        self.at.auth_error.connect(self.handle_auth_error)
        self.at.start()
    ...
    def handle_auth_success(self):
        self.scan_st.setText(tr("scan_connected"))
        self.scan_st.setStyleSheet("color: #28a745; font-weight: bold;")
    
    def handle_auth_error(self, e):
        QMessageBox.critical(self, tr("error"), f"{tr('error_connection', error=e)}")

    def start_scan(self):
        link = self.group_in.text().strip()
        if not link:
            return QMessageBox.warning(self, tr("error"), tr("error_no_group"))
        self.scan_btn.setVisible(False)
        self.stop_scan_btn.setVisible(True)
        self.scan_pr.setVisible(True)
        self.st = ScanThread(self.aid, self.ah, self.sp, link, self.max_spin.value())
        self.st.progress.connect(self.update_scan_progress)
        self.st.content_found.connect(self.show_content)
        self.st.error.connect(self.scan_error)
        self.st.start()
    ...
    def update_scan_progress(self, p, s):
        self.scan_pr.setValue(p)
        self.scan_st.setText(s)
    
    def scan_error(self, e):
        self.reset_scan_ui()
        QMessageBox.critical(self, tr("error"), f"{tr('error_scan', error=e)}")
    ...
    def start_download(self):
        ...
        if not items:
            return QMessageBox.warning(self, tr("error"), tr("error_no_files_selected"))
        path = Path(self.path_in.text())
        path.mkdir(parents=True, exist_ok=True)
        self.show_download_page()
        self.dt = ParallelDownloadThread(self.aid, self.ah, self.sp, items, path)
        self.dt.progress.connect(self.update_dl_progress)
        self.dt.finished.connect(self.dl_finished)
        self.dt.start()

    def update_dl_progress(self, p, f, c, t, speed, eta):
        self.dl_pr.setValue(p)
        self.cur_f_lab.setText(tr("download_current", filename=f))
        self.stats_lab.setText(f"{tr('download_progress', current=c, total=t)} | {tr('download_eta', eta=eta)}")
        self.speed_lab.setText(tr("download_speed", speed=speed))

    def dl_finished(self, d, f):
        self.stop_dl_btn.setVisible(False)
        self.done_b.setVisible(True)
        self.open_f_b.setVisible(True)
        self.dl_st.setText(tr("download_completed"))
        self.stats_lab.setText(tr("download_stats", downloaded=d, failed=f))
    ...
    def reset_to_scan(self):
        ...
        self.done_b.setVisible(False)
        self.open_f_b.setVisible(False)
        self.stop_dl_btn.setVisible(True)
    ...
    def logout(self):
        if QMessageBox.question(self, tr("menu_settings"), tr("confirm_logout")) == QMessageBox.StandardButton.Yes:
            self.settings.clear()
            QMessageBox.information(self, tr("success"), tr("success_logout"))
            self.show_setup_page()
...
def main():
    ...
    if not TELETHON_AVAILABLE:
        return QMessageBox.critical(None, tr("error"), tr("error_telethon_missing")), 1
    window = ModernWindow()
    window.show()
    return app.exec()
